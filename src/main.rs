...
fn initial_positions(n: usize, n_dims: usize) -> Array2<f32> {
...
fn add_edges(n_vertices: usize, max_degree: usize) -> Vec<Edge> {
    let mut rng = thread_rng();
...
fn print_js(pos: Array2<f32>, edges: Vec<Edge>) {
    println!("(function () {{");
...
fn force_graph(pos: &mut Array2<f32>, edges: &[Edge]) {
...
fn main() {
    let matches = clap_app!(forcegraph =>
        (author: "Ed.Cashin@acm.org")
        (about: "trivial force-directed graph arrangement")
        (@arg N: -n +takes_value "Number of vertices")
        (@arg MAX_DEGREE: -m +takes_value "Max degree")
        (@arg DIMS: -d +takes_value "Number of dimensions")
    )
    .get_matches();

    macro_rules! get_arg {
        ($argname:expr, $default:expr) => {
    ...
    let n = get_arg!("N", 10);
    let d = get_arg!("DIMS", 3);
    let m = get_arg!("MAX_DEGREE", 2);

    let mut pos = initial_positions(n, d);
    let edges = add_edges(n, m);
    force_graph(&mut pos, &edges);
